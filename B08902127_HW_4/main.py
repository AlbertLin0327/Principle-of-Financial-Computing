...
class AmericanAsianPuts:
    ...
    def random_path(self):
        ...
        for i in range(1, self.period + 1):
            self.randomWalk[:, i] = self.randomWalk[:, i - 1] * np.exp(
                (self.interest_rate - (self.volatility_rate ** 2) / 2) * self.deltaT
    ...
    def init_maturity(self):
        ...
        self.meanPrice = np.cumsum(self.randomWalk, axis=1) / (
            1 + np.arange(self.period + 1)
        )
...
def main():
    ...
    monteCarloPut = AmericanAsianPuts(
        spot, strike, barrier, time, interest, volatility, period, simulations
    )
    ...
    print(
        round(monteCarloPut.putPrice, 4), round(monteCarloPut.std, 4), round(delta, 4)
    )
