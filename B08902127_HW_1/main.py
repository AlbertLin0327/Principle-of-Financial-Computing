...
class PutOption:
    ...
    def oprion_price_densitivity(self, up_ratio):
        ...
        price_distribution = [
            Decimal(self.stock_price) * Decimal(up_ratio ** self.depth)
        ]

        for i in range(1, self.depth + 1):
            price_distribution.append(
                price_distribution[i - 1] / Decimal(up_ratio ** 2)
            )
    ...
    def put_price(self):
        ...
        wp_up = (up_ratio - np.exp((-self.interest_rate) * self.deltaT)) / (
            up_ratio ** 2 - 1
        )
        ...
        for i in range(1, self.depth):
            wpdf_up.append(
                Decimal(wpdf_up[i - 1])
                * Decimal(wp_down / wp_up)
                * Decimal(factor[i] / factor[i - 1])
            )

            wpdf_down.append(
                Decimal(wpdf_down[i - 1])
                * Decimal(wp_down / wp_up)
                * Decimal(factor[i] / factor[i - 1])
            )
        ...
        uS = Decimal(
            np.sum(
                [
                    Decimal(wpdf_up[i])
                    if price_distribution[i] <= self.strike_price
                    else 0
        ...
        dS = Decimal(
            np.sum(
                [
                    Decimal(wpdf_down[i])
                    if price_distribution[i + 1] <= self.strike_price
                    else 0
                    for i in range(self.depth)
                ]
            )
        ...
        hedge_ratio = (Decimal(uS) - Decimal(dS)) / Decimal(
            self.stock_price * (up_ratio - down_ratio)
        )
...
def main():
    ...
    while True:
        try:
            stock, strike, interest_rate, volatility, time, depth = map(
                float, input().split()
            )
            break
