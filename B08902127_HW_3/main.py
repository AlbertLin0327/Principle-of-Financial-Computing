...
class DownBarrierOption:
    ...
    def set_essentials(self):
        ...
        self.h = math.floor(
            Decimal(np.log(float(self.stock_price / self.barrier_level)))
            / (self.volatility_rate * np.sqrt(self.deltaT))
        )
        self.lmbda = Decimal(np.log(float(self.stock_price / self.barrier_level))) / (
            self.h * self.volatility_rate * np.sqrt(self.deltaT)
        )

        # Calculate the up and down factors
        self.u = Decimal(
            np.exp(self.volatility_rate * self.lmbda * np.sqrt(self.deltaT))
        )
        self.d = Decimal(1 / self.u)

        # Possibility of each move
        self.p_u = Decimal(1 / (2 * self.lmbda ** 2)) + Decimal(
            (self.interest_rate - self.volatility_rate ** 2 / 2)
            * np.sqrt(self.deltaT)
            / (2 * self.lmbda * self.volatility_rate)
        )

        self.p_d = Decimal(1 / (2 * self.lmbda ** 2)) - Decimal(
            (self.interest_rate - self.volatility_rate ** 2 / 2)
            * np.sqrt(self.deltaT)
            / (2 * self.lmbda * self.volatility_rate)
        )
    ...
    def init_stock_price(self):
        ...
        for i in range(2 * self.depth + 1):
            self.option_price[i] = max(
                0, Decimal(self.strike_price - self.stock_price * power[i])
            )
            self.knockout_option_price[i] = max(
                0, Decimal(self.strike_price - self.stock_price * power[i])
            )
    ...
    def back_traversal(self):
        ...
        for i in range(self.depth - 1, -1, -1):
            for j in range(2 * i + 1):
                ...
                self.option_price[j] = (
                    self.p_u * self.option_price[j]
                    + self.p_m * self.option_price[j + 1]
                    + self.p_d * self.option_price[j + 2]
                )

                # Calculate knock out option price
                self.knockout_option_price[j] = (
                    self.p_u * self.knockout_option_price[j]
    ...
    def pricing(self):
        ...
        self.knockout_price = self.knockout_option_price[0] / Decimal(
            np.exp(self.interest_rate * self.time)
        )

        self.regular_option_price = self.option_price[0] / Decimal(
            np.exp(self.interest_rate * self.time)
        )
...
def main():
    ...
    while True:
        try:
            stock, strike, barrier, interest_rate, volatility, time, depth = map(
                float, input().split()
            )
            break
    ...
    barrier_option = DownBarrierOption(
        stock, strike, barrier, interest_rate, volatility, time, depth
    )
